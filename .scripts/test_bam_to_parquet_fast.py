#!/usr/bin/env python3
"""Quick benchmark for BAM to Parquet - minimal options for max speed."""
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python test_bam_to_parquet_fast.py <input.bam> <output_dir>")
        sys.exit(1)
    ...
    print(f"Options: NO PMD, by_reference only, 8 threads for decompression")
    print()
    ...
    stats = convert_sam_bam_to_parquet_pure_cpp(
        ...
        write_by_read=False,  # Only one partition type
        compression_level=3,
        num_threads=8,  # More threads for BAM decompression
        skip_records=0,
        max_records=0,
        calculate_pmd=False,  # Skip PMD calculation
        library_type="ds",
    )
