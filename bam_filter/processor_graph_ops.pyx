...
cdef uint32_t pick_min_edge_weight_elbow(
    ...
    # Dynamic sizing: unique_reads * ~100 edges/read typical for ancient DNA, with 2x load factor margin
    # Min 1M slots, max 128M slots to bound memory usage
    cdef uint64_t estimated_edges = <uint64_t>pool.unique_read_count * 100
    cdef uint64_t hash_capacity = estimated_edges * 2
    if hash_capacity < (1 << 20):
        hash_capacity = 1 << 20  # Min 1M slots (~12MB)
    elif hash_capacity > (1 << 27):
        hash_capacity = 1 << 27  # Max 128M slots (~1.5GB)
...
cdef int build_igraph_direct_from_read_index(
    ...
    # Hash table for edge weights - dynamically sized based on dataset
    cdef uint64_t estimated_edges = <uint64_t>pool.unique_read_count * 100
    cdef uint64_t hash_capacity = estimated_edges * 2
    if hash_capacity < (1 << 20):
        hash_capacity = 1 << 20  # Min 1M slots (~12MB)
